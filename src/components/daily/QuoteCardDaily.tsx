...
export default function QuoteCardDaily({
  quote,
  onNext,
  onAnswer,
  number,
  ...
  const handleAnswer = (index: number) => {
    ...
    try {
      const { dailyModeStats, today } = loadLocalStorage();
      if (dailyModeStats) {
      ...
      if (today) {
        const { guesses, correct_guesses, date } = today;
        const todayDate = new Date().toISOString().split("T")[0];
        const previousQuoteData = today.quoteData;
  ...
  return (
    <QuoteCard>
      {/* Heading */}
      <QuoteCard.Heading>
        Quote {number} / {globals.DAILY_QUOTES}
      </QuoteCard.Heading>
      {/* Quote area */}
